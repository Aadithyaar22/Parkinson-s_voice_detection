...
> honestly-reported evaluation. Deployed live on HuggingFace Spaces.
> Also runnable locally on a MacBook.
>
...
No installation. No account. Just open, record, analyse.
...
```bash
# 🔧 Make the model pickle compatible with your sklearn version
...
> 💡 **First request** downloads wav2vec2-XLS-R (~1.2 GB from HuggingFace)
> and warms the MPS device — ~30 s the first time, ~1 s afterwards.
> On Apple Silicon, embedding extraction runs on GPU automatically.
...
the feature-list filename — it just works.
...
│   ├─ README.md                          Space-specific README + HF metadata
│   ├─ Dockerfile                         container build instructions
│   └─ requirements*.txt                  runtime-only deps
│
├─ .github/workflows/
│   └─ deploy_space.yml                   CI tests → auto-deploy to HF Spaces
│
...
> 🪄 Drop recordings into `Healthy Controls/` and `PD/` subdirectories
> and label inference just works. Use `--labels-csv` for edge cases.
...
git push origin main
        ↓
GitHub Actions: 15 extractor tests + Flask smoke test
        ↓ (if tests pass)
Assembles clean space_build/ from pva2 source files
        ↓
Pushes to HuggingFace Spaces
        ↓
Live at https://huggingface.co/spaces/Aadithya1122/parkinsons-voice-detection
```

The dev repo (`pva2`) is never modified by the deploy process.
The Space is assembled fresh from pva2 files on every successful CI run.
...
| LLM explanations | Groq · Llama 3.3 70B | Fast inference, clinical explanation in plain English |
...
This v2 rebuild — feature extractor fix, multi-corpus training, cross-corpus experiments,<br/>
wav2vec2 deployment, web UI — was led by <b>Aadithya A R</b>, <b>Kenisha P</b> and <b>Yadunandan M Nimbalkar</b>.</sub>
